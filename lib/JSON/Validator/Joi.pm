...
has [qw(format default max min multiple_of regex)] => undef;
has type => 'object';

for my $attr (qw(optional required strict unique)) {
...
sub extend {
  ...
  if ($self->type ne $by->type) {
    die "Cannot extend '@{[$self->type]} by '@{[$by->type]}'";
  }
  elsif ($self->type eq 'array') {
    my $clone = shift->new(%$self, %$by);
    unshift @{$clone->{items}}, @{$self->{items} || []};
    return $clone;
  }
  elsif ($self->type eq 'object') {
    my $clone = shift->new(%$self, %$by);
    unshift @{$clone->{properties}}, @{$self->{properties} || []};
    return $clone;
  }
  else {
    return shift->new(%$self, %$by);
  }
}

sub integer   { shift->type('integer') }
sub iso_date  { shift->date_time }
sub items     { $_[0]->{items} = [ref $_[1] ? @{$_[1]} : @_]; $_[0] }
sub keys      { shift->props(@_) }
...
sub positive  { shift->_type('number')->min(0) }
...
sub _compile_array {
  ...
  $json->{items}           = $self->{items} if ref $self->{items} eq 'ARRAY';
...
=head2 default
  ...
  $self = $self->default($any);
  $any = $self->default;

Used to set a default value for the field.
...
Marks this item as optional, which will not only be the opposite of
L</required> but undefined values are also allowed.
